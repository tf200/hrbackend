...
const getCriticalActionStats = `-- name: GetCriticalActionStats :one
SELECT
...
func (q *Queries) GetCriticalActionStats(ctx context.Context) (GetCriticalActionStatsRow, error) {
	row := q.db.QueryRow(ctx, getCriticalActionStats)
	var i GetCriticalActionStatsRow
	err := row.Scan(
...
const getEmployeeAvailabilityStats = `-- name: GetEmployeeAvailabilityStats :one
...
type GetEmployeeAvailabilityStatsRow struct {
...
func (q *Queries) GetEmployeeAvailabilityStats(ctx context.Context) (GetEmployeeAvailabilityStatsRow, error) {
	row := q.db.QueryRow(ctx, getEmployeeAvailabilityStats)
	var i GetEmployeeAvailabilityStatsRow
	err := row.Scan(
		&i.Active,
		&i.OnLeave,
		&i.Sick,
...
const getPayrollTotalStats = `-- name: GetPayrollTotalStats :one
...
type GetPayrollTotalStatsRow struct {
...
func (q *Queries) GetPayrollTotalStats(ctx context.Context) (GetPayrollTotalStatsRow, error) {
	row := q.db.QueryRow(ctx, getPayrollTotalStats)
	var i GetPayrollTotalStatsRow
	err := row.Scan(
		&i.MonthStart,
		&i.SalaryTotal,
		&i.ZzpTotal,
...
const getRiskRadarStats = `-- name: GetRiskRadarStats :one
WITH current_month AS (
    SELECT
        date_trunc('month', CURRENT_DATE)::date AS month_start,
        (date_trunc('month', CURRENT_DATE) + INTERVAL '1 month - 1 day')::date AS month_end
...
type GetRiskRadarStatsRow struct {
	MonthStart               pgtype.Date `json:"month_start"`
...
func (q *Queries) GetRiskRadarStats(ctx context.Context) (GetRiskRadarStatsRow, error) {
	row := q.db.QueryRow(ctx, getRiskRadarStats)
	var i GetRiskRadarStatsRow
	err := row.Scan(
		&i.MonthStart,
		&i.ContractsEndingThisMonth,
		&i.OverdueTraining,
		&i.LateArrivalsThisMonth,
	)
	return i, err
}

const listOpenShiftCoverage = `-- name: ListOpenShiftCoverage :many
WITH date_range AS (
    ...
    SELECT es.location_id, es.location_name, es.street, es.house_number, es.house_number_addition, es.postal_code, es.city, es.timezone, es.shift_id, es.shift_name, es.shift_date
...
type ListOpenShiftCoverageRow struct {
...
func (q *Queries) ListOpenShiftCoverage(ctx context.Context, days int32) ([]ListOpenShiftCoverageRow, error) {
	rows, err := q.db.Query(ctx, listOpenShiftCoverage, days)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListOpenShiftCoverageRow{}
	for rows.Next() {
		var i ListOpenShiftCoverageRow
		if err := rows.Scan(
			&i.LocationID,
			&i.LocationName,
			&i.Street,
...
const listTeamHealthByDepartment = `-- name: ListTeamHealthByDepartment :many
...
type ListTeamHealthByDepartmentRow struct {
...
func (q *Queries) ListTeamHealthByDepartment(ctx context.Context) ([]ListTeamHealthByDepartmentRow, error) {
	rows, err := q.db.Query(ctx, listTeamHealthByDepartment)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListTeamHealthByDepartmentRow{}
	for rows.Next() {
		var i ListTeamHealthByDepartmentRow
		if err := rows.Scan(
			&i.DepartmentID,
			&i.DepartmentName,
			&i.StaffingPercent,

...
func toCreateSignDocumentParams(req createSignDocumentRequest) domain.CreateSignDocumentParams {
	...
	for _, r := range req.Recipients {
		recipients = append(recipients, domain.CreateSignDocumentRecipientParams{EmployeeID: r.EmployeeID, SigningOrder: r.SigningOrder})
	}
	return domain.CreateSignDocumentParams{Title: req.Title, SourceAttachmentID: req.SourceAttachmentID, Recipients: recipients, RelatedEntityType: req.RelatedEntityType, RelatedEntityID: req.RelatedEntityID, ExpiresAt: req.ExpiresAt}
}

func toSignDocumentFieldParams(inputs []signDocumentFieldInput) []domain.UpsertSignDocumentFieldParams {
	...
	for _, input := range inputs {
		...
		fields = append(fields, domain.UpsertSignDocumentFieldParams{RecipientID: input.RecipientID, Type: input.Type, PageNumber: input.PageNumber, X: input.X, Y: input.Y, Width: input.Width, Height: input.Height, Required: required, Label: input.Label})
	}
	return fields
}

func toSignDocumentSignParams(documentID uuid.UUID, req signSignDocumentRequest, ipAddress, userAgent *string) domain.SignDocumentSignParams {
	values := make([]domain.SignDocumentFieldValueParams, 0, len(req.FieldValues))
	for _, value := range req.FieldValues {
		values = append(values, domain.SignDocumentFieldValueParams{FieldID: value.FieldID, Value: value.Value})
	}
	return domain.SignDocumentSignParams{DocumentID: documentID, SignatureText: req.SignatureText, SignatureImageFileKey: req.SignatureImageFileKey, SaveSignatureForFuture: req.SaveSignatureForFuture, FieldValues: values, ConsentAccepted: req.ConsentAccepted, ConsentText: req.ConsentText, IPAddress: ipAddress, UserAgent: userAgent}
}
